...
export default function QuotationList() {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="capitalize flex justify-center gap-2 items-center">
            <span
              className={`p-1 h-1 rounded-full ${row.getValue("status") === "DRAFT"
                ? "bg-gray-400"
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      const searchableFields = ['id', 'quotation_number', 'client_name', 'status'];
  ...
  return (
    <div className="container mx-auto py-10 bg-white rounded-xl shadow-sm border">
        ...
        <Input
          ...
          onChange={(event) => {
            ...
            const selectedQuotation = quotations.find(
              (quotation) => quotation.quotation_number === value
            );
