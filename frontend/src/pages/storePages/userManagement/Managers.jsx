...
const Managers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error creating/updating branch manager:', error.response?.data || error.message);
    }
  };
  ...
  return (
    <div className="w-full">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Branch Manager Management</h1>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <h3 className="text-lg font-medium leading-6 text-gray-900 mb-4">
              {isUpdating ? 'Update Branch Manager' : 'Create New Branch Manager'}
            </h3>
            <form onSubmit={handleSubmit}>
              ...
              <input
                ...
                onChange={handleInputChange}
                placeholder={isUpdating ? "Leave blank to keep current password" : "Password"}
                className="w-full p-2 mb-4 border border-gray-300 rounded"
                required={!isUpdating}
              />
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredManagers.map((manager) => (
              <tr key={manager.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{manager.id}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{manager.phone_number || 'Not Available'}</td>
