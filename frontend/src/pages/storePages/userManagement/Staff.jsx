...
const Staff = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          ...
          setNewUser({ username: '', email: '', phone_number: '', password: '', is_active: true });
          fetchStaff();
        } catch (error) {
          console.error('Error creating/updating staff:', error.response?.data || error.message);
        }
      };
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredStaff.map((staff) => (
                <tr key={staff.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{staff.username || 'Not Available'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{staff.email}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{staff.phone_number || 'Not available'}</td>
